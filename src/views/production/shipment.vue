<template>
  <div class="shipment" v-if="permission.indexOf('shipment') != -1">
    <div class="aa">
      <!-- 面包屑 -->
      <div class="bb">
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              size="mini"
              v-if="
                scope.row.complete_status == 0 &&
                permission.indexOf('produce_complete_add') != -1
              "
              @click="handleEdit(scope.$index, scope.row)"
              >{{ "出货" }}</el-button
            >
  ...
  methods: {
    handleEdit(index, row) {
      ...
      this.$router.push({
        path: `/sc_shipment?id=${
          row.style_id
        }&TL=${4}&produce_no=${row.produce_no}`,
      });
    },
    onSubmit(val) {
      if (val == 101) {
