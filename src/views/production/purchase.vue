<template>
  <div class="purchase" v-if="permission.indexOf('purchase') != -1">
    <div class="aa">
      <!-- 面包屑 -->
      <div class="bb">
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              size="mini"
              v-if="
                scope.row.procure_status == 1 || scope.row.procure_status == 2
              "
            ...
            <el-button
              ...
              size="mini"
              v-if="
                scope.row.procure_status == 0 &&
                permission.indexOf('produce_order_procure_add') != -1
              "
              @click="handleEdit(scope.$index, scope.row)"
              >{{ "采购" }}</el-button
            >
  ...
  methods: {
    handleEdit(index, row) {
      this.$router.push({
        path: `/sc_purchase?id=${
          row.style_id
        }&TL=${1}&produce_no=${row.produce_no}`,
      });
    },
    onSubmit(val) {
      if (val == 101) {
