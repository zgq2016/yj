<template>
  <div class="productionScheduling" v-if="power.indexOf('B3000300')!=-1">
    ...
    <div class="main">
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="状态:">
          <el-select v-model="formInline.factory_status" clearable placeholder="状态" style="width:120px">
          ...
          <el-table-column align="center" width="80" label="操作">
            <template slot-scope="scope">
              ...
              <el-button
                class="elbtn"
                size="mini"
                v-if="scope.row.factory_status==0"
                @click="handleEdit(scope.$index, scope.row)"
              >{{"排单"}}</el-button>
            </template>
          </el-table-column>
    ...
    <div style="display: flex;justify-content: space-between;align-items: center;">
      <!-- 打印  导出-->
      <div class="btn">
        <el-button v-print="'#printTest'" size="mini" v-if="power.indexOf('B3000500')!=-1">打印</el-button>
        <el-button size="mini" v-if="power.indexOf('B3000600')!=-1">导出</el-button>
      </div>
      <!-- 分页 -->
      <el-pagination
        class="pagination"
    ...
    handleEdit(index, row) {
      ...
      this.$router.push({
        path: `/productionStyle?id=${
          row.style_id
        }&activeNames=${3}&TL=${2}&produce_no=${row.produce_no}`,
      });
    },
    onSubmit() {
